...
def get_new_set_based_ques(utter_list, utter_id):
...
print(args)
args.input_dir = '/home/vardaan/projects/rpp-bengioy/vardaan/CSQA_v3/valid/QA_65/'
args.output_dir = '/home/vardaan/projects/rpp-bengioy/vardaan/QA_65'
...
for filename in os.listdir(os.path.join(args.input_dir)):
    count += 1
    if count % 10 == 0:
        print count
    dialog_json_file = os.path.join(args.input_dir, filename)
    ...
    try:
        ...
        while utter_id < len(utter_list):
            ...
            if 'ques_type_id' not in utter: # ignore answer utterances
                ...
                if len(dialog_list)>=2 and ques_type_id==2 and 'ques_type_id' in dialog_list[-2] and dialog_list[-2]['ques_type_id'] == 4 and 'sec_ques_sub_type' in utter and utter['sec_ques_sub_type']==2: # Secondary question
                    ...
                    new_utter_ques, new_utter_ans = get_new_set_based_ques(utter_list, utter_id)
                    ...
                    if 'The answer count is' in utter_list[utter_id+1]['utterance']:
                        ...
                        utter_id += 1 # compensate for answer utterance

                elif ques_type_id == 4 and utter['is_inc']==1: # remove the set-based ques NOT preceded by sec. ques
                    if (utter_id)>=2 and 'ques_type_id' in utter_list[utter_id-2] and utter_list[utter_id-2]['ques_type_id'] != 2:
                        utter_id += 2 # skip that ques and move on to next ques.
        ...
        with open(os.path.join(args.output_dir, filename), 'wb') as outfile:
